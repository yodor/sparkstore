...
class OptionsList extends BeanList
{
    ...
    public function __construct()
    {
        ...
        $bean->select()->columns(...$bean->columnNames());
        $bean->select()->alias("(SELECT GROUP_CONCAT(vopt.option_value ORDER BY vopt.position ASC SEPARATOR ';' ) FROM variant_options vopt WHERE vopt.parentID = variant_options.voID )", "parameters");
        ...
        $bean->select()->where()->match("parentID", null);
        ...
        $this->setIterator(new SelectQuery($bean->select(), $bean->key(), $bean->table()));
    ...
    public function processInput(): void
    {
        ...
        if ($this->product_filter->isProcessed()) {
            ...
            $this->query->stmt->where()->match("pclsID", null);
        }
        else {
            ...
            if ($this->class_filter->isProcessed()) {
                ...
                $this->query->stmt->where()->match("prodID", null);
