...
class TBICreditPaymentButton extends CreditPaymentButton
{
    protected $store_uid;
    ...
    public function renderButton()
    {
        ...
        $product_id = $this->sellable->getProductID(); //задайте променливата на PHP, която определя продуктовия id
        $product_price = $this->sellable->getPriceInfo()->getSellPrice(); //задайте променливата на PHP, която определя продуктовата цена
        $product_quantity = 1; //задайте променливата на PHP, която определя продуктовата бройка
        $product_name = $this->sellable->getName(); //задайте променливата на PHP, която определя продуктовото име
        ...
        if (($paramstbi['tbi_status'] == 'Yes') && (($product_price > $minprice_tbi) && ($product_price < $maxprice_tbi))) {
                ...
                <table cellspacing="0" cellpadding="0" border="0">
                    <?php if ($tbi_button_position == 1) { ?>
                        <tr>
                            ...
                            <td style="padding:5px !important; background:transparent !important;">
                                <img id="btn_tbiapi" style="cursor:pointer;"
                                     src="https://tbibank.support/calculators/assets/img/custom_buttons/<?php echo $unicid; ?>.png"
            ...
            <?php if ($paramstbi['tbi_container_status'] == 'Yes') { ?>
                <div class="tbi_float" onclick="<?php if ($testmobile = 'No') {
                    echo 'tbiChangeContainer();';
                } ?>">
                ...
                <div class="tbi-label-container">
                    ...
                    <div class="tbi-label-text">
                        ...
                        <div class="tbi-label-text-a"><a href="https://tbibank.support/procedure.php" target="_blank"
                                                         alt="ИНФОРМАЦИЯ ЗА ОНЛАЙН ПАЗАРУВАНЕ НА КРЕДИТ С TBI BANK">ИНФОРМАЦИЯ
                                ЗА ОНЛАЙН ПАЗАРУВАНЕ НА КРЕДИТ!</a></div>
                    </div>
                </div>
